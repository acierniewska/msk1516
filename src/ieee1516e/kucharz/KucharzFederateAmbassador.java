...
public class KucharzFederateAmbassador extends NullFederateAmbassador {
    private KucharzFederate federate;
    ...
    public KucharzFederateAmbassador(KucharzFederate federate) {
    ...
    public void announceSynchronizationPoint(String label, byte[] tag) {
        ...
        if (label.equals(KucharzFederate.READY_TO_RUN))
    ...
    public void receiveInteraction(InteractionClassHandle interactionClass,
            ...
            throws FederateInternalError {
        try {
            if (interactionClass.equals(KucharzFederate.rtiamb.getInteractionClassHandle("HLAinteractionRoot.przekazanieZamowieniaDoKucharza"))) {
                int idStolika = 0;
                String listaPosilkow = "";
                for (ParameterHandle parameter : theParameters.keySet()) {
                    if (parameter.equals(KucharzFederate.rtiamb.getParameterHandle(interactionClass, "listaPosilkow"))) {
                        listaPosilkow = DecoderUtils.decodeString(theParameters.get(parameter));
                    } else if (parameter.equals(KucharzFederate.rtiamb.getParameterHandle(interactionClass, "idStolika"))) {
                        idStolika = DecoderUtils.decodeInteger(theParameters.get(parameter));
                    }
                }
                PrzekazanieZamowieniaEvent event = new PrzekazanieZamowieniaEvent(idStolika, listaPosilkow);
                federate.przekazanieZamowieniaEvents.add(event);
                log("Dostalem zamowienie od stolika " + idStolika + ": " + listaPosilkow + ", czas" + ((HLAfloat64Time) time).getValue());
            } else if (interactionClass.equals(KucharzFederate.rtiamb.getInteractionClassHandle("HLAinteractionRoot.zamkniecieRestauracji"))){
                for (ParameterHandle parameter : theParameters.keySet()) {
                    if (parameter.equals(KucharzFederate.rtiamb.getParameterHandle(interactionClass, "typKomunikatu"))) {
